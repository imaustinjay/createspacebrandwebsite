...
| `WORKSPACE_URL` | Where the commission is posted. Defaults to `https://createspacebrand.online`. Never derived from a request. Public by design — it is in the nav of every page — so it is named in `SECRETS_SCAN_OMIT_KEYS` in `netlify.toml`; without that, secrets scanning finds the value in the build output and fails the deploy. |
